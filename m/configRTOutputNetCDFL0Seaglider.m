...
%  for more details). The returned struct should have the following fields:
...
%  along with this program.  If not, see <http://www.gnu.org/licenses/>.
  ...
  var_attr_list.AD7714Ch0Gain = {
    ...
    'units'         '1'
    'comments'      'Gain assigned to the pressure sensor channel on the AD7714 analog-to-digital converter.'
    '_FillValue'    default_fill_value };

  var_attr_list.AH0_10V = {
    'long_name'     'low voltage battery pack capacity'
    'units'         'A h'
    'comments'      'Capacity of the 10V (low voltage) battery pack (AmpHr).'
    '_FillValue'    default_fill_value };

  var_attr_list.AH0_24V = {
    'long_name'     'high voltage battery pack capacity'
    'units'         'A h'
    'comments'      'Capacity of the 24V (high voltage) battery pack (AmpHr).'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.ALTIM_BOTTOM_PING_dpth = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth of the glider when performing bottom ping.'
    '_FillValue'    default_fill_value };

  var_attr_list.ALTIM_BOTTOM_PING_rnge = {
    'long_name'     'altimeter detected bottom range'
    'units'         'm'
    'comments'      'Altimeter-detected distance to bottom during bottom ping.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.ALTIM_BOTTOM_PING_RANGE = {
    ...
    'units'         'm'
    'comments'      'Range from the presumed apogee depth to ping for the bottom.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.ALTIM_BOTTOM_TURN_MARGIN = {
    ...
    'standard_name' 'distance'
    'units'         'm'
    'comments'      'Distance from the altimeter detected range at which to initiate the apogee maneuver.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.ALTIM_FREQUENCY = {
    ...
    'units'         'kHz'
    'comments'      'Frequency to use for altimeter pings.'
    '_FillValue'    default_fill_value };

  var_attr_list.ALTIM_PING_DELTA = {
    'long_name'     'altimeter ping depth interval'
    'units'         'm'
    'comments'      'Depth interval to repeat ping after a failed altimeter return or confirmation ping return.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.APOGEE_PITCH = {
    ...
    'units'         'degree'
    'comments'      'Intermediate pitch (position of the pitch mass) that Seaglider pitches to between the dive and climb phases to prevent stalling.'
    '_FillValue'    default_fill_value };

  var_attr_list.CALLS = {
    'long_name'     'number of calls'
    'units'         '1'
    'comments'      'Total calls in an attempt to connect on previous dive surfacing.'
    '_FillValue'    default_fill_value };

  var_attr_list.CALL_NDIVES = {
    'long_name'     'call dive step'
    'units'         '1'
    'comments'      'Number of profiles (dive/climb cycles) to perform before attempting communications.'
    '_FillValue'    default_fill_value };

  var_attr_list.CALL_TRIES = {
    'long_name'     'call maximum attempts'
    'units'         '1'
    'comments'      'Maximum number of phone calls to attempt during a surfacing between dives.'
    '_FillValue'    default_fill_value };

  var_attr_list.CALL_WAIT = {
    'long_name'     'call wait time'
    'units'         's'
    'comments'      'wait time between call attempts during a communication session.'
    '_FillValue'    default_fill_value };

  var_attr_list.CAPMAXSIZE = {
    'long_name'     'capture file maximum size'
    'units'         'bytes'
    'comments'      'Maximum size of the capture file to upload (prior to compression).'
    '_FillValue'    default_fill_value };

  var_attr_list.CAPUPLOAD = {
    'long_name'     'capture file upload flag'
    'units'         '1'
    'comments'      'Whether upload capture file (0 = do not upload cap file, 1 = upload cap file).'
    '_FillValue'    default_fill_value };

  var_attr_list.CAP_FILE_SIZE_field01 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 01 of CAP_FILE_SIZE log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.CAP_FILE_SIZE_field02 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 02 of CAP_FILE_SIZE log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.CF8_MAXERRORS = {
    'long_name'     'compact flash maximum errors'
    'units'         '1'
    'comments'      'Maximum number of compact fash errors allowed.'
    '_FillValue'    default_fill_value };

  var_attr_list.CFSIZE_bytes_free = {
    'long_name'     'compact flash free space'
    'units'         'bytes'
    'comments'      'Available free space on the compact flash card.'
    '_FillValue'    default_fill_value };

  var_attr_list.CFSIZE_bytes_total = {
    'long_name'     'compact flash capacity'
    'units'         'bytes'
    'comments'      'Total capacity of the compact flash card.'
    '_FillValue'    default_fill_value };

  var_attr_list.COMM_SEQ = {
    'long_name'     'file transfer sequence'
    'units'         '1'
    'comments'      'Specification of the sequence of file transfer to use.'
    '_FillValue'    default_fill_value };

  var_attr_list.COMPASS2_DEVICE = {
    'long_name'     'second compass device configuration flag'
    'units'         '1'
    'comments'      'Configuration flags specifying the specific model and port for the second compass and transponder/altimeter devices.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.COMPASS_USE = {
    ...
    'units'         '1'
    'comments'      'Parameter used in development to assess compass problems (0 = trust compass inputs, use and report fully calibrated values).'
    '_FillValue'    default_fill_value };

  var_attr_list.COURSE_BIAS = {
    'long_name'     'heading bias compensation'
    'units'         'degree'
    'comments'      'A heading bias to compensate for an observed tendency of Seaglider to veer to one side.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.CURRENT_cur_dir = {
    ...
    'units'         'degree'
    'comments'      'Direction of depth averaged current calculated in some navigation modes.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.CURRENT_cur_spd = {
    ...
    'units'         'm s-1'
    'comments'      'Speed of depth averaged current calculated in some navigation modes.'
    '_FillValue'    default_fill_value };

  var_attr_list.CURRENT_cur_val = {
    'long_name'     'depth averaged current validity'
    'units'         '1'
    'comments'      'Validity check of depth averaged current calculated in some navigation modes (1 = valid, 0 = invalid).'
    '_FillValue'    default_fill_value };

  var_attr_list.C_PITCH = {
    'long_name'     'pitch center position'
    'units'         'counts'
    'comments'      'Flat position for pitch.'
    '_FillValue'    default_fill_value };

  var_attr_list.C_ROLL_CLIMB = {
    'long_name'     'climb roll center position'
    'units'         'counts'
    'comments'      'Straight flight position for roll during climb phase.'
    '_FillValue'    default_fill_value };

  var_attr_list.C_ROLL_DIVE = {
    'long_name'     'dive roll center position'
    'units'         'counts'
    'comments'      'Straight flight position for roll during dive phase.'
    '_FillValue'    default_fill_value };

  var_attr_list.C_VBD = {
    'long_name'     'VBD center position'
    'units'         'counts'
    'comments'      'Neutrally buoyant position for VBD.'
    '_FillValue'    default_fill_value };

  var_attr_list.DATA_FILE_SIZE_bytes = {
    'long_name'     'data file size'
    'units'         'bytes'
    'comments'      'Total size of the data file.'
    '_FillValue'    default_fill_value };

  var_attr_list.DATA_FILE_SIZE_samples = {
    'long_name'     'number of data samples'
    'units'         '1'
    'comments'      'Number of data samples taken during the dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.DEEPGLIDER = {
    'long_name'     'Deepglider type indicator'
    'units'         ''
    'comments'      'Indicates whether the glider is a Deepglider instead of a standard Seaglider (0 = standard Seaglider).'
    '_FillValue'    default_fill_value };

  var_attr_list.DEEPGLIDERMB = {
    'long_name'     'Deepglider motherboard indicator'
    'units'         '1'
    'comments'      'Indicates whether the main board is intended for use in Deepglider (0 = standard Seaglider main board).'
    '_FillValue'    default_fill_value };

  var_attr_list.DEVICE1 = {
    'long_name'     'device slot 1 configuration flag'
    'units'         '1'
    'comments'      'Configuration flag specifying device type and port for each of the six possible attached science sensors. Empty device slots are indicated with a parameter value of -1.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_ABORT = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Maximum depth for Seaglider operations.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_CALL = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'GPS acquisition and Iridium phone call starting depth.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_FINISH = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth to finish the dive.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_FLARE = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth to flare to the computed pitch angle following the initial dive from the surface.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_GRID = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Bathymetry map depth where apogee state begins.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_NO_BLEED = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth below which no oil bleeds from the bladder.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_OFFGRID = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Bathymetry map depth value used outside the map area.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_PITCH = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth to reach before surface pitch mneuver.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_SAFE = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Target depth for escape route flight and VBD depth safety limit.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_SURF = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth to surface approach.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.D_TGT = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth to reach to start apogee phase.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_bufoverrun = {
    'long_name'     'buffer overrun errors'
    'units'         '1'
    'comments'      'Number of times the log file output is longer than the internal buffer length.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_fclos_errs = {
    'long_name'     'close file errors'
    'units'         '1'
    'comments'      'Number of CF8 errors while closing files.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_fclos_rets = {
    'long_name'     'close file retries'
    'units'         '1'
    'comments'      'Number of CF8 retries while closing files.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_fopen_errs = {
    'long_name'     'open file errors'
    'units'         '1'
    'comments'      'Number of CF8 errors while opening files.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_fopen_rets = {
    'long_name'     'open file retries'
    'units'         '1'
    'comments'      'Number of CF8 retries while opening files.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_fwrit_errs = {
    'long_name'     'write file errors'
    'units'         '1'
    'comments'      'Number of CF8 errors while writing files.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_fwrit_rets = {
    'long_name'     'write file retries'
    'units'         '1'
    'comments'      'Number of CF8 retries while writing files.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_gps_mis = {
    'long_name'     'gps missing errors'
    'units'         '1'
    'comments'      'Number of times GPS did not provide data records within the 2 second timeout.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_gps_pps = {
    'long_name'     'gps pulse per second errors'
    'units'         '1'
    'comments'      'Number of GPS pulse per second (PPS) errors.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_interrupts = {
    'long_name'     'interrupt errors'
    'units'         '1'
    'comments'      'Number of spurious interrupts resultin from division by zero, memory dereference problems, or interrupt contention.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_ptch_errs = {
    'long_name'     'pitch errors'
    'units'         '1'
    'comments'      'Number of pitch errors.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_ptch_rets = {
    'long_name'     'pitch retries'
    'units'         '1'
    'comments'      'Number of pitch retries.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_roll_errs = {
    'long_name'     'roll errors'
    'units'         '1'
    'comments'      'Number of roll errors.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_roll_rets = {
    'long_name'     'roll retries'
    'units'         '1'
    'comments'      'Number of roll retries.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_vbd_errs = {
    'long_name'     'VBD errors'
    'units'         '1'
    'comments'      'Number of VBD errors.'
    '_FillValue'    default_fill_value };

  var_attr_list.ERRORS_vbd_rets = {
    'long_name'     'VBD retries'
    'units'         '1'
    'comments'      'Number of VBD retries.'
    '_FillValue'    default_fill_value };

  var_attr_list.ESCAPE_HEADING = {
    'long_name'     'escape heading'
    'units'         'degree'
    'comments'      'Base heading to steer in an escape recovery situation when either no position fix is available or no escape target in targets file.'
    '_FillValue'    default_fill_value };

  var_attr_list.ESCAPE_HEADING_DELTA = {
    'long_name'     'eascape heading increment'
    'units'         'degree'
    'comments'      'Escape heading increment according to bottom depth change.'
    '_FillValue'    default_fill_value };

  var_attr_list.FERRY_MAX = {
    'long_name'     'maximum rhumb line correction'
    'units'         'degree'
    'comments'      'Maximum correction to apply to the rhumb line to the next waypoint when NAV_MODE = 2.'
    '_FillValue'    default_fill_value };

  var_attr_list.FG_AHR_10V = {
    'long_name'     'low voltage battery pack consumption'
    'units'         'A h'
    'comments'      'Cumulative power consumed from the low voltage battery pack (10 V) as tracked by the supervisor fuel gauge.'
    '_FillValue'    default_fill_value };

  var_attr_list.FG_AHR_10Vo = {
    'long_name'     'low voltage battery pack dive end consumption'
    'units'         'A h'
    'comments'      'Cumulative power consumed from the low voltage battery pack (24 V) as tracked by the supervisor fuel gauge and recorded at the end of the dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.FG_AHR_24V = {
    'long_name'     'high voltage battery pack consumption'
    'units'         'A h'
    'comments'      'Cumulative power consumed from the high voltage battery pack (24 V) as tracked by the supervisor fuel gauge.'
    '_FillValue'    default_fill_value };

  var_attr_list.FG_AHR_24Vo = {
    'long_name'     'high voltage battery pack dive end consumption'
    'units'         'A h'
    'comments'      'Cumulative power consumed from the high voltage battery pack (24 V) as tracked by the supervisor fuel gauge and recorded at the end of the dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.FILEMGR = {
    'long_name'     'file management agressivity'
    'units'         '1'
    'comments'      'Onboard file system management agressivity (0 = none, 1 = only store compressed files, 2 = delete splits on failed phone call).'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.FINISH_dpth = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Depth of glider at the first sample taken after reaching D_SURF (or D_FINISH, if enabled).'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.FIX_MISSING_TIMEOUT = {
    ...
    'units'         'days'
    'comments'      'Maximum time allowed without any valid navigation fix (GPS, RAFOS, Iridium geolocation) before entering into recovery state.'
    '_FillValue'    default_fill_value };

  var_attr_list.GLIDE_SLOPE = {
    'long_name'     'maximum glide slope'
    'units'         'degree'
    'comments'      'Absolute value of the maximum glide slope allowed.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.GPSFIX_ddmmyy = {
    ...
    'comments'      'Day of month (dd), month of year (mm), 2-digit year (yy) UTC'
    '_FillValue'    0 };
  ...
  var_attr_list.GPSFIX_fixlat = {
    ...
    'units'         'nmea_degree'
    'comments'      'Latitude coordinate of GPS fix.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.GPSFIX_fixlon = {
    ...
    'units'         'nmea_degree'
    'comments'      'Longitude coordinate of GPS fix.'
    '_FillValue'    default_fill_value };

  var_attr_list.GPSFIX_hhmmss = {
    'long_name'     'gps time stamp'
    'comments'      'Hour (hh), minute (mm), second (ss) UTC'
    '_FillValue'    0 };

  var_attr_list.GPSFIX_hordop = {
    'long_name'     'horizontal dilution of precision'
    'units'         ''
    'comments'      'Measure of the strength of the figure used to compute the GPS fix.'
    '_FillValue'    default_fill_value };

  var_attr_list.GPSFIX_magvar = {
    'long_name'     'gps magnetic variation'
    'units'         'degree'
    'comments'      'Magnetic variation, positive east.'
    '_FillValue'    default_fill_value };

  var_attr_list.GPSFIX_ttafix = {
    'long_name'     'gps time to acquire fix'
    'units'         ''
    'comments'      'Total time to acquire fix.'
    '_FillValue'    default_fill_value };

  var_attr_list.GPSFIX_ttffix = {
    'long_name'     'gps time to first fix'
    'units'         's'
    'comments'      'Time to first fix,'
    '_FillValue'    default_fill_value };

  var_attr_list.HD_A = {
    'long_name'     'lift coefficient'
    'units'         ''
    'comments'      'Hydrodynamic parameter representing the lift coefficient determined empirically and used in onboard performance prediction and guidance calculations.'
    '_FillValue'    default_fill_value };

  var_attr_list.HD_B = {
    'long_name'     'drag coefficient'
    'units'         ''
    'comments'      'Hydrodynamic parameter representing the drag coefficient determined empirically and used in onboard performance prediction and guidance calculations.'
    '_FillValue'    default_fill_value };

  var_attr_list.HD_C = {
    'long_name'     'induced drag coefficient'
    'units'         ''
    'comments'      'Hydrodynamic parameter representing the induced drag coefficient determined empirically and used in on-board performance prediction and guidance calculations.'
    '_FillValue'    default_fill_value };

  var_attr_list.HEADING = {
    'long_name'     'navigation heading'
    'units'         'degree'
    'comments'      'Syntetic waypoint heading (NAV_MODE 0,1 or 2) or heading offset from depth-averaged current (NAV_MODE 3).'
    '_FillValue'    default_fill_value };

  var_attr_list.HEAD_ERRBAND = {
    'long_name'     'heading deadband'
    'units'         'degree'
    'comments'      'Heading minimum error to recquire a correction in guidance and control mode.'
    '_FillValue'    default_fill_value };

  var_attr_list.HEAPDBG = {
    'long_name'     'heap debug control'
    'units'         '1'
    'comments'      'Control of the extended heap debugging function.'
    '_FillValue'    default_fill_value };

  var_attr_list.HUMID = {
    'long_name'     'internal humidity'
    'units'         '%'
    'comments'      'Relative humidity inside the pressure hull.'
    '_FillValue'    default_fill_value };

  var_attr_list.ICE_FREEZE_MARGIN = {
    'long_name'     'freezing point temperature margin'
    'units'         'Celsius'
    'comments'      'Temperature margin used in the freezing point calculation.'
    '_FillValue'    default_fill_value };

  var_attr_list.ID = {
    'long_name'     'identification number'
    'units'         '1'
    'comments'      'Seglider identification (serial) number.'
    '_FillValue'    default_fill_value };

  var_attr_list.INTERNAL_PRESSURE = {
    'long_name'     'internal pressure'
    'units'         'psia'
    'comments'      'Pressure inside the pressure hull'
    '_FillValue'    default_fill_value };

  var_attr_list.INT_PRESSURE_SLOPE = {
    'long_name'     'internal pressure calibration slope'
    'units'         'psia counts-1'
    'comments'      'Slope calibration of the internal pressure sensor.'
    '_FillValue'    default_fill_value };

  var_attr_list.INT_PRESSURE_YINT = {
    'long_name'     'internal pressure calibration intercept'
    'units'         'psia'
    'comments'      'Y-intercept calibration of the internal pressure sensor.'
    '_FillValue'    default_fill_value };

  var_attr_list.IRIDIUM_FIX_field01 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 01 of IRIDIUM_FIX log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.IRIDIUM_FIX_field02 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 02 of IRIDIUM_FIX log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.IRIDIUM_FIX_field03 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 03 of IRIDIUM_FIX log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.IRIDIUM_FIX_field04 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 04 of IRIDIUM_FIX log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_CONTROL_spd_east = {
    'long_name'     'desired eastward speed'
    'units'         'm s-1'
    'comments'      'Eastward component of horizontal speed from which heading is derived.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_CONTROL_spd_nrth = {
    'long_name'     'desired northward speed'
    'units'         'm s-1'
    'comments'      'Northward component of horizontal speed from which heading is derived.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_USE = {
    'long_name'     'navigation Kalman filter control'
    'units'         '1'
    'comments'      'Control parameter for the run state of the Kalman filter navigation program (0 = reset Kalman filter, 1 = run Kalman filter, 2 = do not run the Kalman filter).'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_X_cur_diur_east = {
    'long_name'     'Kalman filter diurnal current easting'
    'units'         'm'
    'comments'      'East position relative to initial position at time tk due to diurnal current.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_X_cur_mean_east = {
    'long_name'     'Kalman filter mean current easting'
    'units'         'm'
    'comments'      'East position relative to initial position at time tk due to mean current.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_X_cur_semi_east = {
    'long_name'     'Kalman filter semidiurnal current easting'
    'units'         'm'
    'comments'      'East position relative to initial position at time tk due to semidiurnal current.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_X_delta_x = {
    'long_name'     'Kalman filter x displacement'
    'units'         'm'
    'comments'      'X displacement from present position to predicted position due to mean, diurnal and semidiurnal components of the model.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_X_gld_wspd_east = {
    'long_name'     'Kalman filter glider speed easting'
    'units'         'm'
    'comments'      'East position relative to initial position at time tk due to glider speed through water.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_Y_cur_diur_nrth = {
    'long_name'     'Kalman filter diurnal current northing'
    'units'         'm'
    'comments'      'North position relative to initial position at time tk due to diurnal current.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_Y_cur_mean_nrth = {
    'long_name'     'Kalman filter mean current northing'
    'units'         'm'
    'comments'      'North position relative to initial position at time tk due to mean current.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_Y_cur_semi_nrth = {
    'long_name'     'Kalman filter semidiurnal current northing'
    'units'         'm'
    'comments'      'NOrth position relative to initial position at time tk due to semidiurnal current.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_Y_delta_y = {
    'long_name'     'Kalman filter y displacement'
    'units'         'm'
    'comments'      'Y displacement from present position to predicted position due to mean, diurnal and semidiurnal components of the model.'
    '_FillValue'    default_fill_value };

  var_attr_list.KALMAN_Y_gld_wspd_nrth = {
    'long_name'     'Kalman filter glider speed northing'
    'units'         'm'
    'comments'      'North position relative to initial position at time tk due to glider speed through water.'
    '_FillValue'    default_fill_value };

  var_attr_list.KERMIT = {
    'long_name'     'kermit control'
    'units'         ''
    'comments'      'Control of the use of kermit (vs xmodem) for file transfers. Not used.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.LOGGERS = {
    ...
    'units'         ''
    'comments'      'bit mask for the autonomous logging devices configured with the Autonomous Logger Interface (ALI).'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.MASS = {
    ...
    'standard_name' 'mass'
    'units'         'g'
    'comments'      'Mass used in onboard buoyancy and current estimation calculation.'
    '_FillValue'    default_fill_value };

  var_attr_list.MAX_BUOY = {
    'long_name'     'maximum negative thrust'
    'units'         'cm3'
    'comments'      'Absolute value of the maximum negative thrust (in cc) that Seaglider is allowed to develop during the dive phase.'
    '_FillValue'    default_fill_value };

  var_attr_list.MEM = {
    'long_name'     'free RAM memory'
    'units'         'bytes'
    'comments'      'Amount of free RAM reported by the glider.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.MINV_10V = {
    ...
    'units'         'V'
    'comments'      'minimum allowable observed voltage on the low voltage battery pack before going into recovery.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.MISSION = {
    ...
    'units'         '1'
    'comments'      'Current mission number.'
    '_FillValue'    default_fill_value };

  var_attr_list.MOTHERBOARD = {
    'long_name'     'motherboard revision'
    'units'         '1'
    'comments'      'Revision of the carried motherboard.'
    '_FillValue'    default_fill_value };

  var_attr_list.NAV_MODE = {
    'long_name'     'navigation mode'
    'units'         '1'
    'comments'      'Heading selection method (0 = constant HEADING, 1 = KALMAN_USE, 2 = ferry angle correction wrt depth-averaged current, 3 = HEADING relative to depth-averaged current).'
    '_FillValue'    default_fill_value };

  var_attr_list.N_FILEKB = {
    'long_name'     'data upload file size'
    'units'         'kbytes'
    'comments'      'Size and type of file used for data uploading (disable gzip compression if negative).'
    '_FillValue'    default_fill_value };

  var_attr_list.N_GPS = {
    'long_name'     'gps maximum time'
    'units'         's'
    'comments'      'Time to wait after the first GPS fix is received for a GPS fix with HDOP less than 2.0.'
    '_FillValue'    default_fill_value };

  var_attr_list.N_NOCOMM = {
    'long_name'     'allowed no-communication dives'
    'units'         '1'
    'comments'      'Number of dives allowed without a complete and successful data communication session before setting the surface buoyancy parameter to the allowed maximum.'
    '_FillValue'    default_fill_value };

  var_attr_list.N_NOSURFACE = {
    'long_name'     'non-surfacing dive control'
    'units'         '1'
    'comments'      'Control of surfacing and non-surfacing dives. If positive dives multiple of N_NOSURFACE will complete at subsurface instead of surface. A negative value reverses this logic.'
    '_FillValue'    default_fill_value };

  var_attr_list.PHONE_DEVICE = {
    'long_name'     'phone device model'
    'units'         '1'
    'comments'      'Model of the attached phone device.'
    '_FillValue'    default_fill_value };

  var_attr_list.PHONE_SUPPLY = {
    'long_name'     'phone power supply'
    'units'         ''
    'comments'      'Iridium modem power usage attribution (1 = 10V battery, 2 = 24V battery).'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_ADJ_DBAND = {
    'long_name'     'pitch control deadband'
    'units'         'degree'
    'comments'      'Allowed deadband for active pitch control during dive and climb.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_ADJ_GAIN = {
    'long_name'     'pitch control gain'
    'units'         'cm degree-1'
    'comments'      'Adjustment factor for active pitch control during dive and climb.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_AD_RATE = {
    'long_name'     'pitch rate retry threshold'
    'units'         'count s-1'
    'comments'      'Pitch rate threshold for retries when pitching.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_CNV = {
    'long_name'     'pitch position conversion factor'
    'units'         'cm counts-1'
    'comments'      'Pitch position conversion factor set by manufacturer or assembler.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_DBAND = {
    'long_name'     'pitch deadband'
    'units'         'cm'
    'comments'      'Pitch position deadband (cm) within which no further pitch motion is commanded.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_GAIN = {
    'long_name'     'pitch gain'
    'units'         'degree cm-1'
    'comments'      'Amount of vehicle pitch change corresponding to a 1 cm movement of the pitch mass.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_MAX = {
    'long_name'     'pitch maximum position'
    'units'         'counts'
    'comments'      'Pitch position software limit aft.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_MAXERRORS = {
    'long_name'     'maximum pitch errors'
    'units'         '1'
    'comments'      'Number of pitch motor errors allowed before going into recovery state.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_MIN = {
    'long_name'     'pitch minimum position'
    'units'         'counts'
    'comments'      'Pitch position software limit forward.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_TIMEOUT = {
    'long_name'     'pitch timeout'
    'units'         's'
    'comments'      'Time allowed the mass shifter to achieve the desired pitch position.'
    '_FillValue'    default_fill_value };

  var_attr_list.PITCH_VBD_SHIFT = {
    'long_name'     'pitch VBD compensation'
    'units'         'cm cm-3'
    'comments'      'Pitch compensation (cm/cm3) required to balance the mass of the hydraulic oil moving forward and aft with the change in buoyancy as a result of VBD changes.'
    '_FillValue'    default_fill_value };

  var_attr_list.PRESSURE_SLOPE = {
    'long_name'     'pressure slope'
    'units'         ''
    'comments'      'Slope of linear fit between psig and pressure sensor output (after digitization to A/D counts through AD7714).'
    '_FillValue'    default_fill_value };

  var_attr_list.PRESSURE_YINT = {
    'long_name'     'pressure interecept'
    'units'         ''
    'comments'      'Y-intercept of linear fit between psig and pressure sensor output (after digitization to A/D counts through AD7714).'
    '_FillValue'    default_fill_value };

  var_attr_list.P_OVSHOOT = {
    'long_name'     'pitch overshoot'
    'units'         'cm'
    'comments'      'Distance by which the pitch mass is allowed to overshoot its target after the pitch motor is turned off.'
    '_FillValue'    default_fill_value };

  var_attr_list.RAFOS_CORR_THRESH = {
    'long_name'     'RAFOS correlation threshold'
    'units'         '1'
    'comments'      'Correlation threshold to use when selecting RAFOS hits for navigation solutions.'
    '_FillValue'    default_fill_value };

  var_attr_list.RAFOS_DEVICE = {
    'long_name'     'RAFOS device flag'
    'units'         '1'
    'comments'      'Model of the attached RAFOS device (-1 = RAFOS device not installed).'
    '_FillValue'    default_fill_value };

  var_attr_list.RAFOS_HIT_WINDOW = {
    'long_name'     'RAFOS hit window size'
    'units'         '1'
    'comments'      'Size of search window in seconds to use when clustering hits for navigation solutions.'
    '_FillValue'    default_fill_value };

  var_attr_list.RAFOS_PEAK_OFFSET = {
    'long_name'     'RAFOS arrival time offset'
    'units'         's'
    'comments'      'Offset of actual arrival time from the receiver reported arrival index due to receiver firmware artefacts.'
    '_FillValue'    default_fill_value };

  var_attr_list.RELAUNCH = {
    'long_name'     'post-error relaunch flag'
    'units'         '1'
    'comments'      'Behaviour in the event of a crash to TOM8 or any watchdog timer reset (0 = resume diving, 1 = enter recovery)'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.RHO = {
    ...
    'standard_name' 'sea_water_density'
    'units'         'kg l-1'
    'comments'      'Water density value used for converting buoyancy force in grams to sea water displacement in cm3 and on-board performance prediction computations.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_ADJ_DBAND = {
    'long_name'     'roll control deadband'
    'units'         'degree s-1'
    'comments'      'Allowed deadband for roll control.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_ADJ_GAIN = {
    'long_name'     'roll control gain'
    'units'         'degree degree-1 s-1'
    'comments'      'Adjustment factor for roll control.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_AD_RATE = {
    'long_name'     'roll rate retry threshold.'
    'units'         'counts s-1'
    'comments'      'Roll rate threshold for retries when rolling.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_CNV = {
    'long_name'     'roll position conversion factor'
    'units'         'degree counts-1'
    'comments'      'Roll position conversion factor determined by the particulars of the roll gear train, motor and potentiometer.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_DEG = {
    'long_name'     'turn roll angle'
    'units'         'degree'
    'comments'      'Angle to roll the internal roll mass during a turn.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_MAX = {
    'long_name'     'roll maximum position'
    'units'         'counts'
    'comments'      'Roll position software limit to starboard.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_MAXERRORS = {
    'long_name'     'maximum roll errors'
    'units'         '1'
    'comments'      'Number of roll motor errors allowed before going into recovery state.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_MIN = {
    'long_name'     'roll minimum position'
    'units'         'counts'
    'comments'      'Roll position software limit to port.'
    '_FillValue'    default_fill_value };

  var_attr_list.ROLL_TIMEOUT = {
    'long_name'     'roll timeout'
    'units'         's'
    'comments'      'Time allowed to achieve the desired roll position.'
    '_FillValue'    default_fill_value };

  var_attr_list.R_PORT_OVSHOOT = {
    'long_name'     'roll port overshoot'
    'units'         'counts'
    'comments'      'Roll mass overshoot to port after motor is turned off.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.SEABIRD_C_G = {
    ...
    'units'         ''
    'comments'      'Seabird Electronics-provided calibration coefficients for conductivity sensor used to compute salinity and density for self-trimming applications and subsurface finish maneuvers.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.SEABIRD_T_G = {
    ...
    'units'         ''
    'comments'      'Seabird Electronics-provided calibration coefficients for conductivity sensor used to compute temperature, salinity and density for self-trimming applications and subsurface finish maneuvers.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.SENSOR_MAMPS_AA4330 = {
    ...
    'units'         'mA'
    'comments'      'Maximum current drawn by Aanderaa 4330 sensor.'
    '_FillValue'    default_fill_value };

  var_attr_list.SENSOR_MAMPS_SBE_CT = {
    'long_name'     'seabird ct maximum current'
    'units'         'mA'
    'comments'      'Maximum current drawn by Seabird conductivity and temperature sail.'
    '_FillValue'    default_fill_value };

  var_attr_list.SENSOR_MAMPS_WL_BBFL2VMT = {
    'long_name'     'bbfl2 triplet maximum current'
    'units'         'mA'
    'comments'      'Maximum current drawn by WETLabs BBFL2 fluorometer and backscatter sensor.'
    '_FillValue'    default_fill_value };

  var_attr_list.SENSOR_MAMPS_field04 = {
    'long_name'     ''
    'units'         'mA'
    'comments'      'Unknown sensor or nil (no sensor present in this slot).'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.SENSOR_SECS_AA4330 = {
    ...
    'units'         's'
    'comments'      'Time Aanderaa 4330 sensor.'
    '_FillValue'    default_fill_value };

  var_attr_list.SENSOR_SECS_SBE_CT = {
    'long_name'     'seabird ct power time'
    'units'         's'
    'comments'      'Time Seabird conductivity and temperature sail.'
    '_FillValue'    default_fill_value };

  var_attr_list.SENSOR_SECS_WL_BBFL2VMT = {
    'long_name'     'bbfl2 power time'
    'units'         's'
    'comments'      'Time WETLabs BBFL2 fluoreometer and backscatter sensor.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.SIM_PITCH = {
    ...
    'units'         'degree'
    'comments'      'Simulated pitch angle value during the dive phase of a simulated run.'
    '_FillValue'    default_fill_value };

  var_attr_list.SIM_W = {
    'long_name'     'simulated vertical velocity'
    'units'         'm s-1'
    'comments'      'Simulated vertical velocity during the dive phase of a simulated run.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_ANGLEo = {
    'long_name'     'surface angle'
    'units'         'degree'
    'comments'      'Glider-measured angle at surface at end of previous dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_CC = {
    'long_name'     'VBD minimum buoyancy position'
    'units'         'cm3'
    'comments'      'Specified minimum-buoyancy position of the VBD to attain at surface.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.SM_DEPTHo = {
    ...
    'units'         'm'
    'positive'      'down'
    'comments'      'Glider-measured depth at surface at end of previous dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field01 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 01 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field02 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 02 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field03 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 03 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field04 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 04 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field05 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 05 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field06 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 06 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field07 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 07 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field08 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 08 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field09 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 09 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field10 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 10 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field11 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 11 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field12 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 12 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_GC_field13 = {
    'long_name'     ''
    'units'         ''
    'comments'      'Unknown field 13 SM_GC log parameter.'
    '_FillValue'    default_fill_value };

  var_attr_list.SPEED_FACTOR = {
    'long_name'     'speed compensation factor'
    'units'         '1'
    'comments'      'factor to compensate the inability to dive at the desired horizontal velocity.'
    '_FillValue'    default_fill_value };

  var_attr_list.SPEED_LIMITS_max_spd = {
    'long_name'     'maximum horizontal speed'
    'units'         'm s-1'
    'comments'      'Maximum attainable horizontal speed on dive based on the minimum dive angle and the allowable buoyancy force.'
    '_FillValue'    default_fill_value };

  var_attr_list.SPEED_LIMITS_min_spd = {
    'long_name'     'minimum horizontal speed'
    'units'         'm s-1'
    'comments'      'Minimum attainable horizontal speed on dive based on the maximum dive angle and the allowable buoyancy force.'
    '_FillValue'    default_fill_value };

  var_attr_list.STROBE = {
    'long_name'     'strobe led control'
    'units'         '1'
    'comments'      'Control of the blinking of the strobe led (0 = no blinking, 1 = on recovery, 2 = on surface).'
    '_FillValue'    default_fill_value };

  var_attr_list.SURFACE_URGENCY = {
    'long_name'     'extra surfacing allowed dives'
    'units'         '1'
    'comments'      'Dives to accumulate before trying extra surfacings. For ice capable gliders only.'
    '_FillValue'    default_fill_value };

  var_attr_list.SURFACE_URGENCY_FORCE = {
    'long_name'     'forced extra surfacing dive modulo'
    'units'         '1'
    'comments'      'Dive number modulo for forced extra surfacing attempts. For under ice only.'
    '_FillValue'    default_fill_value };

  var_attr_list.SURFACE_URGENCY_TRY = {
    'long_name'     'extra surfacing dive modulo'
    'units'         '1'
    'comments'      'Dive number modulo for extra surfacing attempts. For under ice only.'
    '_FillValue'    default_fill_value };

  var_attr_list.TCM_PITCH_OFFSET = {
    'long_name'     'pitch-compass offset'
    'units'         'degree'
    'comments'      'Static offset in pitch axis between the compass output and the actual glider body asmeasured in the lab.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.TCM_TEMP = {
    ...
    'units'         'Celsius'
    'comments'      'Last temperature reading taken from compass.'
    '_FillValue'    default_fill_value };

  var_attr_list.TGT_AUTO_DEFAULT = {
    'long_name'     'default target update control'
    'units'         '1'
    'comments'      'Control of automatic update of default target (1 = automatically update default target in NVRAM, 0 = do not update default target in NVRAM).'
    '_FillValue'    default_fill_value };

  var_attr_list.TGT_DEFAULT_LAT = {
    'long_name'     'default target latitude'
    'units'         'nmea_degree'
    'comments'      'Latitude coordinate of default target location used when the targets file cannot be read.'
    '_FillValue'    default_fill_value };

  var_attr_list.TGT_DEFAULT_LON = {
    'long_name'     'default target longitude'
    'units'         'nmea_degree'
    'comments'      'Longitude coordinate of default target location used when the targets file cannot be read.'
    '_FillValue'    default_fill_value };

  var_attr_list.TGT_LATLONG_tgt_lat = {
    'long_name'     'target latitude'
    'units'         'nmea_degree'
    'comments'      'Latitude coordinate of target position for current dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.TGT_LATLONG_tgt_lon = {
    'long_name'     'target longitude'
    'units'         'nmea_degree'
    'comments'      'Longitude coordinate of target position of current dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.TGT_NAME = {
    'long_name'     'target name'
    'units'         ''
    'comments'      'Name of active target of this dive.'
    '_FillValue'    0 };

  var_attr_list.TGT_RADIUS = {
    'long_name'     'target radius'
    'units'         'm'
    'comments'      'Radius of active target of this dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.TT8_MAMPS = {
    'long_name'     'tt8 end dive power'
    'units'         'A'
    'comments'      'Power draw on the low voltage battery pack (10 V) at the end of the dive (it can be used to determine if devices are being left on).'
    '_FillValue'    default_fill_value };

  var_attr_list.T_ABORT = {
    'long_name'     'dive timeout'
    'units'         'minutes'
    'comments'      'Time allowed for a dive before entering the recovery state.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_BOOST = {
    'long_name'     'boost pump time'
    'units'         's'
    'comments'      'Boost pump run time before turning it off when deeper than boost depth.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_DIVE = {
    'long_name'     'dive time'
    'units'         'minutes'
    'comments'      'Time to make one dive-climb cycle (not including pump during apogee phase)used to calculate the desired vertical velocity.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_GPS = {
    'long_name'     'GPS timeout'
    'units'         'minutes'
    'comments'      'Maximum allowed time to obtain a GPS position.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_GPS_ALMANAC = {
    'long_name'     'GPS almanac timeout'
    'units'         'minutes'
    'comments'      'Maximum allowed time to acquire the GPS almanac.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_GPS_CHARGE = {
    'long_name'     'Garmin GPS25 trickle-charging time'
    'units'         'secs'
    'comments'      'Time to wait before trickle-charging the GPS receiver (for Garmin GPS25 engines only). GPS units now installed run on a button battery so there is no need to charge the system.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_LOITER = {
    'long_name'     'loiter time'
    'units'         's'
    'comments'      'Time to loiter after going neutral at apogee, before pitching up and becoming positively buoyant for climb.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_MISSION = {
    'long_name'     'mission timeout'
    'units'         'minutes'
    'comments'      'Maximum mission time allowed.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_NO_W = {
    'long_name'     'no vertical speed time'
    'units'         's'
    'comments'      'Wait time with no significantly non-zero vertical velocity (less than 1 cm/s, as measured by dP/dt) before proceeding to the next phase of a dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_RSLEEP = {
    'long_name'     'recovery sleep time interval'
    'units'         'minutes'
    'comments'      'Sleep time interval during the recovery phase.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_TURN = {
    'long_name'     'turn timeout'
    'units'         's'
    'comments'      'maximum time allowed to complete a turn during the active GC mode.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_TURN_SAMPINT = {
    'long_name'     'turn sampling interval'
    'units'         's'
    'comments'      'Sample interval during active and passive GC while turning.'
    '_FillValue'    default_fill_value };

  var_attr_list.T_WATCHDOG = {
    'long_name'     'watchdog timer'
    'units'         'minutes'
    'comments'      'Value to which the watchdog timer is set. This is an information only parameter so the glider software knows the watchdog timer value.'
    '_FillValue'    default_fill_value };

  var_attr_list.UNCOM_BLEED = {
    'long_name'     'VBD uncommanded bleed trigger'
    'units'         'counts'
    'comments'      'Uncommanded change of VBD bleed that triggers the safety actions (stop motors, close skinner valve) and entering into recovery state.'
    '_FillValue'    default_fill_value };

  var_attr_list.UPLOAD_DIVES_MAX = {
    'long_name'     'maximum dive upload'
    'units'         ''
    'comments'      'maximum number of dives to upload at one surfacing (-1 = upload all available dives that have not been previously uploaded)'
    '_FillValue'    default_fill_value };

  var_attr_list.USE_BATHY = {
    'long_name'     'bathymetry map use control'
    'units'         '1'
    'comments'      'Control of use of onboard bathymap to determine the appropiate apogee depth (-4 = use on-board bathymap appropiate for current position, 0 = do not use bathymetry, positive = use matching bathymap).'
    '_FillValue'    default_fill_value };

  var_attr_list.USE_ICE = {
    'long_name'     'ice map use control'
    'units'         '1'
    'comments'      'Control of use of onboard icemap for surfacing decisions (-4 = use on-board bathymap appropiate for current position, 0 = do not use bathymetry, positive = use matching bathymap).'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_BLEED_AD_RATE = {
    'long_name'     'bleed rate retry threshold'
    'units'         'counts s-1'
    'comments'      'Bleed rate threshold for retries when bleeding.'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_CNV = {
    'long_name'     'VBD position conversion factor'
    'units'         'cm3 counts-1'
    'comments'      'VBD position conversion factor determined by the geometry of the internal hydraulic fluid reservoir and the potentiometers (the sign is negative, meaning that the higher A/D counts reflect more oil in the internal reservoir, hence a less inflated external bladder, hence a lower displacement, and thus a lower buoyancy).'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_DBAND = {
    'long_name'     'VBD position deadband'
    'units'         'cm3'
    'comments'      'Allowed deadband for VBD control.'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_MAX = {
    'long_name'     'VBD maximum position'
    'units'         'counts'
    'comments'      'VBD position software limit when the internal reservoir is almost full (external bladder fully bled, minimum buoyancy).'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_MAXERRORS = {
    'long_name'     'maximum VBD errors'
    'units'         '1'
    'comments'      'Number of VBD errors allowed before entering into recovery state.'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_MIN = {
    'long_name'     'VBD minimum position'
    'units'         'counts'
    'comments'      'VBD position software limit when the internal reservoir is almost empty (external bladder fully pumped, maximum buoyancy).'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_PUMP_AD_RATE_APOGEE = {
    'long_name'     'apogee pump rate'
    'units'         'counts s-1'
    'comments'      'Pump rate threshold for retries when pumping at apogee.'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_PUMP_AD_RATE_SURFACE = {
    'long_name'     'apogee pump rate'
    'units'         'counts s-1'
    'comments'      'Pump rate threshold for retries when pumping at surface.'
    '_FillValue'    default_fill_value };

  var_attr_list.VBD_TIMEOUT = {
    'long_name'     'VBD timeout'
    'units'         's'
    'comments'      'Time allowed to achieve the commanded change in VBD position.'
    '_FillValue'    default_fill_value };

  var_attr_list.XMS_NAKs = {
    'long_name'     'no acknowledged transfers'
    'units'         '1'
    'comments'      'Number of transfers ended with a NAK (No Acknowledgements) on previous surfacing.'
    '_FillValue'    default_fill_value };

  var_attr_list.XMS_TOUTs = {
    'long_name'     'no timeout transfers'
    'units'         '1'
    'comments'      'Total number of transfers ended without a timeout on previous surfacing.'
    '_FillValue'    default_fill_value };

  var_attr_list.XPDR_DEVICE = {
    'long_name'     'transponder model flag'
    'units'         '1'
    'comments'      'Configuration value specifying the model of the attached device.'
    '_FillValue'    default_fill_value };

  var_attr_list.XPDR_INHIBIT = {
    'long_name'     'transponder inhibit time'
    'units'         'ds'
    'comments'      'Time after a transponder reply during which the transponder does not reply to subsequent interrogation.'
    '_FillValue'    default_fill_value };

  var_attr_list.XPDR_PINGS = {
    'long_name'     'transponder pings'
    'units'         '1'
    'comments'      'Count of the number of times the glider transducer responded to an external stimulus at its frequency setting during a dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.XPDR_VALID = {
    'long_name'     'transponder validation sensitivity'
    'units'         '0.5 ms'
    'comments'      'Total time over a 10 ms window following initial triggering that the detector circuit must remain triggered.'
    '_FillValue'    default_fill_value };

  var_attr_list.x10V_AH_ampsh_tot = {
    'long_name'     'low voltage battery pack total consumption'
    'units'         'A h'
    'comments'      'Total power consumed on the low voltage battery pack (10 V) since last reset of battery meters (usually when new batteries are installed).'
    '_FillValue'    default_fill_value };

  var_attr_list.x10V_AH_volts_min = {
    'long_name'     'low voltage battery pack minimum voltage'
    'units'         'V'
    'comments'      'Minimum voltage measured on the low voltage battery pack (10 V) during active phase.'
    '_FillValue'    default_fill_value };

  var_attr_list.x24V_AH_ampsh_tot = {
    'long_name'     'high voltage battery pack total consumption'
    'units'         'A h'
    'comments'      'Total power consumed on the high voltage battery pack (24 V) since last reset of battery meters (usually when new batteries are installed).'
    '_FillValue'    default_fill_value };

  var_attr_list.x24V_AH_volts_min = {
    'long_name'     'high voltage battery pack minimum voltage'
    'units'         'V'
    'comments'      'Minimum voltage measured on the high voltage battery pack (10 V) during active phase.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_data_pts = {
    'long_name'     'data points'
    'units'         '1'
    'comments'      'Number of data records collected thus far in the dive.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_end_secs = {
    'long_name'     'gc end elapsed time'
    'units'         's'
    'comments'      'Elapsed time from the start of the dive to the end of the GC.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_ob_vertv = {
    'long_name'     'observed vertical velocity'
    'units'         'cm s-1'
    'comments'      'Observed vertical velocity.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_phase = {
    'long_name'     'gc phase'
    'units'         '1'
    'comments'      'Guidance and control phase (1: pitch change, 2: VBD change, 3: roll, 4: turning (passive), 5: roll back to center, 6: passive mode waiting).'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_pitch_ad = {
    'long_name'     'gc pitch position'
    'units'         'counts'
    'comments'      'Position of the pitch motor at the end of the motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_pitch_ctl = {
    'long_name'     'gc pitch control'
    'units'         'cm'
    'comments'      'Position of the pitch mass relative to pitch center (positive aft).'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_pitch_errors = {
    'long_name'     'gc pitch errors'
    'units'         '1'
    'comments'      'Number of pitch motor errors (timeouts) during this motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_pitch_i = {
    'long_name'     'gc pitch motor current'
    'units'         'A'
    'comments'      'Average current used by pitch motor.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_pitch_retries = {
    'long_name'     'gc pitch motor retries'
    'units'         '1'
    'comments'      'Number of retries during this pitch motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_pitch_secs = {
    'long_name'     'gc pitch motor time'
    'units'         's'
    'comments'      'Time the pitch motor was on.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_roll_ad = {
    'long_name'     'gc roll position'
    'units'         'counts'
    'comments'      'Position of the roll motor at the end of the motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_roll_errors = {
    'long_name'     'gc roll errors'
    'units'         '1'
    'comments'      'Number of roll motor errors (timeouts) during this motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_roll_i = {
    'long_name'     'gc roll motor current'
    'units'         'A'
    'comments'      'Average current used by roll motor.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_roll_retries = {
    'long_name'     'gc roll motor retries'
    'units'         '1'
    'comments'      'Number of retries during this roll motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_roll_secs = {
    'long_name'     'gc roll motor time'
    'units'         's'
    'comments'      'Time the roll motor was on.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_vbd_ad = {
    'long_name'     'gc VBD position'
    'units'         'counts'
    'comments'      'Position of the VBD at the end of the motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_vbd_ctl = {
    'long_name'     'gc VBD control'
    'units'         'cm3'
    'comments'      'Position of the VBD mass relative to VBD center (positive buoyant).'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_vbd_errors = {
    'long_name'     'gc VBD errors'
    'units'         '1'
    'comments'      'Number of VBD errors (timeouts) during this motor move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_vbd_i = {
    'long_name'     'gc VBD current'
    'units'         'A'
    'comments'      'Average current used by VBD.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_vbd_retries = {
    'long_name'     'gc VBD retries'
    'units'         '1'
    'comments'      'Number of retries during this VBD move.'
    '_FillValue'    default_fill_value };

  var_attr_list.GC_vbd_secs = {
    'long_name'     'gc VBD time'
    'units'         's'
    'comments'      'Time the VBD was on.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_CCo_pmp_amps = {
    'long_name'     'sm VBD current'
    'units'         'A'
    'comments'      'Average current for the VBD during the surface maneuver pump.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_CCo_pmp_ccss = {
    'long_name'     'sm VBD final position'
    'units'         'cm3'
    'comments'      'Final position of the VBD after the surface maneuver pump.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.SM_CCo_pmp_errs = {
    ...
    'units'         '1'
    'comments'      'Number of errors during the surface maneuver pump.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_CCo_pmp_rets = {
    'long_name'     'sm VBD retries'
    'units'         '1'
    'comments'      'Number of retries during the surface maneuver pump.'
    '_FillValue'    default_fill_value };

  var_attr_list.SM_CCo_pmp_secs = {
    'long_name'     'sm VBD time'
    'units'         's'
    'comments'      'Time for the surface maneuver pump.'
    '_FillValue'    default_fill_value };

  var_attr_list.STATE_result = {
    'long_name'     'gc result state'
    'units'         ''
    'comments'      'State result at end of phase.'
    '_FillValue'    0 };

  var_attr_list.STATE_status = {
    'long_name'     'gc state action'
    'units'         ''
    'comments'      'Current state action.'
    '_FillValue'    0 };
  ...
  var_attr_list.depth = {
    ...
    'units'         'cm'
    'positive'      'down'
    'comments'      'This is the depth derived from the pressure measurements and using a constant conversion factor (0.685 psi m-1).'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.elaps_t = {
    ...
    'comments'      'Time since the start of the first dive (time_coverage_start).'
    '_FillValue'    default_fill_value };

  var_attr_list.elaps_t_0000 = {
    'long_name'     'time since start of day'
    'units'         's'
    'comments'      'Time since 0000UTC of the current day.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.head = {
    ...
    'units'         'degree'
    'comments'      'Vehicle heading (magnetic degree).'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.pitchAng = {
    ...
    'standard_name' 'pitch'
    'units'         'degree'
    'comments'      'Vehicle pitch angle (positive nose up).'
    '_FillValue'    default_fill_value };

  var_attr_list.pitchCtl = {
    'long_name'     'pitch mass position'
    'units'         'cm'
    'comments'      'Pitch mass position relative to pitch center (positive nose up).'
    '_FillValue'    default_fill_value };

  var_attr_list.rec = {
    'long_name'     'sample record'
    'units'         '1'
    'comments'      'Sample record number.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.rollAng = {
    ...
    'standard_name' 'roll'
    'units'         'degree'
    'comments'      'Vehicle roll angle (positive starboard wing down, rolled to starboard).'
    '_FillValue'    default_fill_value };

  var_attr_list.rollCtl = {
    'long_name'     'roll mass position'
    'units'         'degree'
    'comments'      'Roll mass position relative to roll dive or climb center (positive starboard wing down).'
    '_FillValue'    default_fill_value };

  var_attr_list.vbdCC = {
    'long_name'     'VBD position'
    'units'         'cm3'
    'comments'      'VBD value relative to VBD center (positive buoyant).'
    '_FillValue'    default_fill_value };

  var_attr_list.sbect_condFreq = {
    'long_name'     'conductivity frequency'
    'units'         'Hz'
    'comments'      'Conductivity frequency from Seabird CT sail.'
    '_FillValue'    default_fill_value };

  var_attr_list.sbect_tempFreq = {
    'long_name'     'temperature frequency'
    'units'         'Hz'
    'comments'      'Temperature frequency from Seabird CT sail.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.aa1_AirSat = {
    ...
    'units'         '%'
    'comments'      'Air saturation from Aanderaa oxygen optode.'
    '_FillValue'    default_fill_value };

  var_attr_list.aa1_CalPhase = {
    'long_name'     'optode calibrated phase'
    'units'         'degree'
    'comments'      'Calibrated phase from Aanderaa oxygen optode.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.aa1_O2 = {
    ...
    'units'         'umol l-1'
    'comments'      'Oxygen molarity from Aanderaa oxygen optode.'
    '_FillValue'    default_fill_value };
  ...
  var_attr_list.aa1_Temp = {
    ...
    'units'         'Celsius'
    'comments'      'Temperature from Aanderaa oxygen optode.'
    '_FillValue'    default_fill_value };
