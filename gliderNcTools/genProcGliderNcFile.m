function genProcGliderNcFile(outFilename, processedData, params)
%GENPROCGLIDERNCFILE - Generates a glider netcdf file from processed data
% This function creates, defines and fills in a netcdf file with glider data
%
% Syntax: genProcGliderNcFile(outFilename, processedData, params)
%
% Inputs:
%    outFilename - Fully qualified name of the netcdf output file
%    processedData - structure containing the fields to be converted to
%    variables
...
%
% Example:
%    genProcGliderNcFile(outFilename, processedData, params)
%
...
%% VARIABLES
    % Get the list of fields that are time series
    fieldList = processedData.timeseries;
    ...
    for fieldIdx = 1:length(fieldList)
        ...
        varstruct.Attribute = getProcGliderVarAtts(currentFieldName);
        nc_addvar(outFilename, varstruct);
    end

%% GLOBAL ATTRIBUTES: METADATA
    % A list of 'fixed' attributes (user can override them)
    nc_attput(outFilename, nc_global, 'type', 'Slocum glider data file');
    ...
    if isfield(processedData, 'source') && ~isempty(processedData.source)
        sourceFileList = processedData.source{1};
        for srcIdx = 2:length(processedData.source)
            sourceFileList = strcat(sourceFileList, ['; ', processedData.source{srcIdx}]);
        end;
...
%% FILE FILLING
    ...
    for fieldIdx = 1:length(fieldList)
        ...
        currentFieldName = fieldList{fieldIdx};
        currentFieldData = processedData.(currentFieldName);
        nc_varput(outFilename, currentFieldName, ...
            currentFieldData, 0, length(currentFieldData));
    end
